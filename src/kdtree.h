	...
	class KDTree{
		...
		private:
			...
			*/
			...
			void radiusSearchRecursive(const Eigen::Vector2d& query, const Node* node, std::vector<std::pair<double, int>>& set, double radius) const
			{
				if (node == nullptr)
					return;
				...
				const double dist = distance(query, train, dim_);
				if (dist < radius)
					set.push_back(std::make_pair(dist, node->idx));

				const int axis = node->axis;
				const int dir = query[axis] < train[axis] ? 0 : 1;
				radiusSearchRecursive(query, node->next[dir], set, radius);
