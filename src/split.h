...
pair<IteratorType_, IntVector::iterator> split(IteratorType_ begin,
                    IteratorType_ end,
                    IntVector::iterator indices_begin,
                    IntVector::iterator indices_end,
  ...
  auto indices_lower = indices_begin;
  auto indices_upper = std::make_reverse_iterator(indices_end);

  while (lower!=upper.base()) {
    ValueType& v_lower=*lower;
    ValueType& v_upper=*upper;
    int& i_lower = *indices_lower;
    int& i_upper = *indices_upper;
    if ( predicate(v_lower) ){
      ...
      std::swap(i_lower, i_upper);
      ++indices_upper;
    }
  }
  return make_pair(upper.base(), indices_upper.base());
