...
int main (int argc, char** argv) {
    ...
    for (size_t pose_index = 0; pose_index < map.size(); pose_index++)
    {
      ...
      {
        MapPoint& map_point  = map[pose_index][point_index];
        map_point.set_points_2dtree_index(pose_index);
    ...
    for (size_t pose_index = 0; pose_index < map.size(); pose_index++)
    {
      ...
      bool pose_found = finder.find_pose_neighbors(pose_index, 5, pose_neighbors, 2);
        ...
        if (pose_found) 
        {
          ...
          for (size_t i = 0; i < pose_neighbors.size(); i++)
          {
              ...
              if (tree_index != pose_index){
                ...
                if (finder.find_point_neighbor(tree_index, map_point.pose_index(), map_point.point_index(), map_point.normal_index(), point_neighbor, 0.25))
                {
